...
function run-tomcats
{
  ...
  docker run -d \
    --name tomcat-1 \
    -v $(pwd):/jmx-exporter \
    -e CATALINA_OPTS="-Xms64m -Xmx64m -javaagent:/jmx-exporter/jmx_prometheus_javaagent-0.3.1.jar=6060:/jmx-exporter/jmx-exporter-config.yml -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.port=1100 -Dcom.sun.management.jmxremote.rmi.port=1100 -Djava.rmi.server.hostname=localhost" \
    -p 6060:6060 \
    -p 8080:8080 \
    -p 1100:1100 \
    tomcat:8.5-alpine
